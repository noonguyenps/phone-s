...
public interface ProductRepository extends JpaRepository<ProductEntity, UUID> {
    ...
    @Query(value = "SELECT * FROM products WHERE LOWER(converttvkdau(products.product_name)) LIKE '%?1%' OR LOWER(converttvkdau(products.product_name)) LIKE '%?2%' AND products.product_status=1",
            countQuery = "SELECT * FROM products WHERE LOWER(converttvkdau(products.product_name)) LIKE '%?1%' OR LOWER(converttvkdau(products.product_name)) LIKE '%?2%' AND products.product_status=1",
            nativeQuery = true)
